<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="mb-8 font-jakarta">
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Pengguna</h1>
            <p class="text-slate-500 font-medium italic text-sm">Memperbarui informasi akun untuk {{ $user->name }}.</p>
        </div>
        ...
        <div class="bg-white rounded-[40px] p-8 md:p-12 shadow-sm border border-slate-50">
            <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <!-- Role -->
                    <div class="col-span-2">
                        <label for="role" class="block text-sm font-semibold text-slate-700 mb-3">Role Pengguna</label>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <label class="relative flex items-center p-4 bg-slate-50 rounded-2xl cursor-pointer hover:bg-slate-100 transition-all border-2 border-transparent has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50/50 group">
                                <input type="radio" name="role" value="dosen" class="hidden" {{ old('role', $user->role) == 'dosen' ? 'checked' : '' }} required>
                            ...
                            <label class="relative flex items-center p-4 bg-slate-50 rounded-2xl cursor-pointer hover:bg-slate-100 transition-all border-2 border-transparent has-[:checked]:border-emerald-600 has-[:checked]:bg-emerald-50/50 group">
                                <input type="radio" name="role" value="mahasiswa" class="hidden" {{ old('role', $user->role) == 'mahasiswa' ? 'checked' : '' }}>
                    ...
                    <!-- Security Info -->
                    <div class="col-span-2 pt-6 border-t border-slate-50">
                        <div class="flex items-center space-x-3 mb-6">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-slate-800 leading-none">Keamanan Akun</h3>
                                <p class="text-xs font-medium text-slate-400 mt-1 italic">Kosongkan kolom password jika tidak ingin mengubahnya.</p>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <label for="password" class="block text-sm font-semibold text-slate-700 mb-3">Password Baru</label>
