<x-app-layout>
    <x-breadcrumbs :links="[
        ...
        ['label' => 'Jurnal Penyesuaian', 'url' => route('mahasiswa.jurnal-penyesuaian.index')],
        ['label' => 'Edit Jurnal']
    ]" />
    ...
    <div x-data="journalEntry()" class="pb-24 w-full" x-init="init()">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            <div>
                <span class="inline-block px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold mb-3">Adjustment Entry</span>
                <h1 class="text-3xl font-black text-slate-900 tracking-tight">Perbarui Jurnal Penyesuaian</h1>
                <p class="text-slate-500 font-medium text-sm mt-1">Mengubah data penyesuaian akhir periode.</p>
            </div>
            <div class="hidden md:block text-right">
                <p class="text-xs font-bold text-slate-400 leading-none mb-1">Status Keseimbangan</p>
                <div :class="isBalanced ? 'text-blue-600' : 'text-slate-300'" class="flex items-center justify-end font-black transition-colors">
                    <span x-text="isBalanced ? 'Seimbang' : 'Belum Seimbang'" class="text-xl"></span>
                    <div :class="isBalanced ? 'bg-blue-500 shadow-lg shadow-blue-200' : 'bg-slate-200'" class="w-2.5 h-2.5 rounded-full ml-3 animate-pulse"></div>
                </div>
            </div>
        </div>

        <form action="{{ route('mahasiswa.jurnal-penyesuaian.update', $jurnal) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden w-full">
                <div class="bg-slate-50/50 px-8 py-5 border-b border-slate-100">
                    <h2 class="text-sm font-bold text-slate-700">Informasi Jurnal</h2>
                </div>
                <div class="p-8">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                        <div class="md:col-span-3">
                            <label for="tanggal" class="block text-sm font-bold text-slate-500 mb-2 px-1">Tanggal</label>
                            <input type="text" name="tanggal" id="tanggal" value="{{ \Carbon\Carbon::parse($jurnal->tanggal)->format('Y-m-d') }}" class="datepicker w-full px-4 py-2.5 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:border-blue-600 focus:ring-0 transition-all font-bold text-slate-700 text-sm" required placeholder="Pilih Tanggal">
                        </div>

                        <div class="md:col-span-3">
                            <label for="nomor_bukti" class="block text-sm font-bold text-slate-500 mb-2 px-1">Nomor Bukti</label>
                            <input type="text" name="nomor_bukti" id="nomor_bukti" value="{{ $jurnal->nomor_bukti }}" class="w-full px-4 py-2.5 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:border-blue-600 focus:ring-0 transition-all font-bold text-slate-700 text-sm" placeholder="AJP-001" required>
                        </div>

                        <div class="md:col-span-6">
                            <label for="keterangan" class="block text-sm font-bold text-slate-500 mb-2 px-1">Keterangan Jurnal</label>
                            <input type="text" name="keterangan" id="keterangan" value="{{ $jurnal->keterangan }}" class="w-full px-4 py-2.5 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:border-blue-600 focus:ring-0 transition-all font-bold text-slate-700 text-sm" placeholder="Contoh: Penyesuaian beban perlengkapan" required>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden w-full">
                <div class="bg-slate-50/50 px-8 py-5 border-b border-slate-100 flex items-center justify-between">
                    <h2 class="text-sm font-bold text-slate-700">Item Penyesuaian</h2>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                            ...
                            <template x-for="(row, index) in rows" :key="index">
                                <tr class="group hover:bg-slate-50/30 transition-colors">
                                    ...
                                    <td class="px-8 py-2.5">
                                        <input type="text" 
                                               :id="'debit-' + index"
                                               class="nominal-input w-full px-3 py-2 bg-slate-50 border-transparent rounded-lg focus:bg-white focus:border-blue-600 focus:ring-0 transition-all text-right text-sm font-bold text-slate-800"
                                               placeholder="0">
                        ...
                        <tfoot class="bg-blue-900 text-white">
                            <tr>
                                <td class="px-8 py-4">
                                    <button type="button" @click="addRow()" class="inline-flex items-center text-xs font-bold text-blue-200 hover:text-white transition-colors">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                        Tambah baris baru
                                    </button>
                                </td>
                                <td class="px-8 py-4 text-right">
                                    <p class="text-[10px] font-bold text-blue-300 leading-none mb-1">Total Debit</p>
                                    <p class="text-xl font-black" x-text="'Rp ' + formatNumber(totalDebit)"></p>
                                </td>
                                <td class="px-8 py-4 text-right">
                                    <p class="text-[10px] font-bold text-blue-300 leading-none mb-1">Total Kredit</p>
                ...
                <div class="w-full flex items-center justify-between bg-white/95 backdrop-blur-sm border border-slate-200 rounded-2xl p-4 shadow-2xl">
                    <div class="flex items-center space-x-12 px-6">
                        <div class="flex flex-col">
                            ...
                            <p class="text-2xl font-black transition-all" :class="isBalanced ? 'text-blue-500' : 'text-red-500'" x-text="'Rp ' + formatNumber(Math.abs(totalDebit - totalKredit))"></p>
                        </div>
                        
                        <div class="h-10 w-px bg-slate-100 hidden md:block"></div>
                        
                        <div class="flex items-center space-x-4">
                            <div :class="isBalanced ? 'bg-blue-100 text-blue-600' : 'bg-red-50 text-red-500'" 
                            ...
                            <div>
                                ...
                                <p :class="isBalanced ? 'text-blue-600' : 'text-red-500'" class="text-sm font-black" x-text="isBalanced ? 'Jurnal Seimbang' : 'Belum Seimbang'"></p>
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center space-x-4 pr-6">
                        <a href="{{ route('mahasiswa.jurnal-penyesuaian.index') }}" class="px-6 py-3.5 text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">Batal</a>
                        <button type="submit" 
                                :disabled="!isBalanced"
                                :class="!isBalanced ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-50' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-100'"
                                class="px-12 py-4 rounded-xl text-sm font-black transition-all flex items-center group/btn shadow-sm">
                            <span>Perbarui Jurnal Penyesuaian</span>
