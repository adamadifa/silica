<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Jurnal Umum</h1>
            <p class="text-slate-500 font-medium text-xs">Riwayat seluruh pencatatan transaksi akuntansi perusahaan.</p>
        </div>
        @if(!request()->has('perusahaan_id'))
        <div>
            <a href="{{ route('mahasiswa.jurnal.create') }}" class="flex items-center px-6 py-3 bg-indigo-600 rounded-2xl text-sm font-bold text-white hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 group">
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($jurnals as $jurnal)
                        ...
                        <tr class="bg-slate-50/20 group/row">
                            ...
                            <td class="px-8 py-4 text-xs font-bold text-indigo-600 border-r border-slate-50 text-center">
                                {{ $jurnal->nomor_bukti }}
                            </td>
                            <td class="px-8 py-4 text-sm font-black text-slate-800" colspan="3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center space-x-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                        <a href="{{ route('mahasiswa.jurnal.edit', $jurnal) }}" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all" title="Edit Transaksi">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form action="{{ route('mahasiswa.jurnal.destroy', $jurnal) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus transaksi ini? Tindakan ini tidak dapat dibatalkan.')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all" title="Hapus Transaksi">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-8 py-20 text-center">
                                ...
                                <p class="text-slate-400 font-medium">Belum ada transaksi jurnal yang dicatat.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
