<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
            <div class="flex items-center space-x-2 text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">
                <span class="px-2 py-0.5 bg-blue-50 rounded italic whitespace-nowrap">Workspace Aktif</span>
                ...
                <span class="flex items-center">
                    ...
                    Periode: {{ \Carbon\Carbon::createFromDate($perusahaan->current_year, $perusahaan->current_month, 1)->format('F Y') }}
                </span>
            </div>
            ...
            <p class="text-slate-500 font-medium text-xs mt-1">
                Periode Akuntansi: {{ \Carbon\Carbon::parse($perusahaan->periode_awal)->format('d M Y') }} - {{ \Carbon\Carbon::parse($perusahaan->periode_akhir)->format('d M Y') }}
            </p>
        </div>
        
        <div class="flex items-center space-x-3">
            <button class="px-6 py-3 bg-white border border-slate-100 rounded-2xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
                Pengaturan
            </button>
            <form action="{{ route('mahasiswa.perusahaan.closing', $perusahaan) }}" method="POST">
                @csrf
                <button type="submit" class="px-6 py-3 bg-indigo-600 rounded-2xl text-sm font-bold text-white hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <!-- COA Card -->
        <a href="{{ route('mahasiswa.coa.index', $perusahaan) }}" class="bg-white p-8 rounded-[32px] border border-slate-50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group">
        ...
        <!-- Jurnal Card -->
        <a href="{{ route('mahasiswa.jurnal.index', $perusahaan) }}" class="bg-white p-8 rounded-[32px] border border-slate-50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-4 bg-blue-600 rounded-[40px] p-10 text-white flex flex-col justify-between">
            <div>
                <h3 class="text-xl font-bold mb-4">Butuh Bantuan?</h3>
                <p class="text-blue-100 text-xs leading-relaxed">
                    Jika Anda bingung menentukan posisi Debit/Kredit, silakan cek modul panduan akuntansi atau tanyakan pada Dosen pengampu.
                </p>
            </div>
            <div class="mt-10">
                <button class="w-full py-4 bg-white/10 hover:bg-white/20 border border-white/20 rounded-2xl text-xs font-bold transition-all">
                    Lihat Panduan COA
                </button>
            </div>
        </div>
